...
static const char blankString[] = QT_TRANSLATE_NOOP("serial_console", "N/A");

serial_console::serial_console(QWidget *parent ):
									 QDialog (parent),
                                     ui_console (new Ui::serial_console),
...
// initialize and connect serial port objects
  ...
  connect(serial, static_cast<void (QSerialPort::*)(QSerialPort::SerialPortError)>
	                         (&QSerialPort::error),
	      this, &serial_console::handleError);
  connect(serial, &QSerialPort::readyRead, this, &serial_console::readData);
  ...
  connect(ui_console->comboBox_serialInfo,
	  ...
	  &serial_console::showPortInfo);

}

void serial_console::connectCOM()
{
...
void serial_console::disconnectCOM()
{
	if (serial->isOpen())
		serial->close();
...
void serial_console::emumeratePorts()
{
...
void serial_console::showPortInfo(int idx)
{
	if (idx == -1)
		return;
...
void serial_console::keyPressEvent(QKeyEvent *e)
{
    switch (e->key()) {
    ...
    default:
        if (localEchoEnabled)
			serial_console::keyPressEvent(e);
        emit getData(e->text().toLocal8Bit());
    }
}

void serial_console::mousePressEvent(QMouseEvent *e)
{
    Q_UNUSED(e)
    setFocus();
}

void serial_console::mouseDoubleClickEvent(QMouseEvent *e)
{
    Q_UNUSED(e)
}

void serial_console::contextMenuEvent(QContextMenuEvent *e)
{
    Q_UNUSED(e)
}

void serial_console::readData()
{
	if (serial->isReadable()  ) {

		//QByteArray data = serial->readAll(); // / readLine();//->readAll();
		//m_data.append(serial->readAll());
		m_data.append(serial->readLine(100));
		string str = m_data.toStdString();
		ui_console->textEdit_console->append(QString::fromStdString(str));
		m_data.clear();

	}
}

void serial_console::handleError(QSerialPort::SerialPortError error)
{
